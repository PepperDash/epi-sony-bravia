...
namespace SonyBraviaEpi
{
    public class SonyBraviaFactory : EssentialsPluginDeviceFactory<SonyBraviaDevice>
    {
        ...
        {
            Debug.Console(DebugLevels.Info, "[{0}] Building {1} plugin instance...", dc.Name, dc.Type);

            var props = dc.Properties.ToObject<SonyBraviaConfig>();
            if (props == null)
            {
                Debug.Console(DebugLevels.Info, "[{0}] Failed to build {1} plugin", dc.Name, dc.Type);
                return null;
            }
            ...
            Debug.Console(DebugLevels.Info, "[{0}] Failed to build {1} plugin using {2}", dc.Name, dc.Type, props.Control.Method);
            return null;
        }
    }
}